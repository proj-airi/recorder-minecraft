...
def _required_scene_world_coordinate(value: Any, description: str) -> int:
    coordinate = _required_int(value, description)
    if not MIN_SCENE_WORLD_COORDINATE <= coordinate <= MAX_SCENE_WORLD_COORDINATE:
        raise SceneStoreError(
            f"{description} must be between {MIN_SCENE_WORLD_COORDINATE} and "
            f"{MAX_SCENE_WORLD_COORDINATE}"
        )
    return coordinate
...
def _blob_uncompressed_size(
    ...
    if (
        ...
        or not 0 <= size <= MAX_BLOB_UNCOMPRESSED_BYTES
    ):
        raise SceneStoreValidationError(
            f"scene blob {sha256} uncompressed size is outside the allowed bounds"
        )
    return size
...
def validate_scene_attachment_provenance(
    ...
    if (
        info.extraction.metadata_policy == SCENE_METADATA_POLICY_FULL
        and not info.sensitive
    ):
        raise SceneStoreValidationError(
...
def _store_info(
    ...
    if (
        ...
        and extraction.metadata_policy == SCENE_METADATA_POLICY_FULL
        and not sensitive
    ):
        raise SceneStoreValidationError(
            "full_packet_metadata scene stores must be marked sensitive"
...
class SceneStore:
    ...
    def _bounded_scene_object_rows(
        ...
        *,
        ...
        if len(rows) > max_count:
            raise SceneStoreError(
                f"scene crop {label} count exceeds the limit of {max_count}"
            )
        total_payload_bytes = 0
        for row in rows:
            digest = row["blob_sha256"]
            payload_bytes = _blob_uncompressed_size(
                self._connection, digest, expected_kind=expected_kind
            )
            if payload_bytes > MAX_CROP_OBJECT_PAYLOAD_BYTES:
                raise SceneStoreError(
                    f"scene crop {label} payload {digest} contains {payload_bytes} "
                    f"bytes; per-object limit is {MAX_CROP_OBJECT_PAYLOAD_BYTES}"
                )
            total_payload_bytes += payload_bytes
            if total_payload_bytes > MAX_CROP_OBJECT_PAYLOAD_TOTAL_BYTES:
                raise SceneStoreError(
                    f"scene crop {label} payloads contain more than "
                    f"{MAX_CROP_OBJECT_PAYLOAD_TOTAL_BYTES} bytes in total"
                )
        return rows
    ...
    def materialize_crop(
        ...
        for index in range(3):
            ...
            if (
                not SQLITE_INTEGER_MIN <= origin[index] <= SQLITE_INTEGER_MAX
                or not SQLITE_INTEGER_MIN <= maximum <= SQLITE_INTEGER_MAX
            ):
                raise SceneStoreError(
                    "scene crop bounds must fit signed 64-bit SQLite integers"
                )
    ...
    def slice(
        ...
        if (
            ...
            or not 0 <= radius <= MAX_SLICE_RADIUS
        ):
            raise SceneStoreError(
                f"scene slice radius must be an integer from 0 to {MAX_SLICE_RADIUS}"
            )
        coordinate = _required_scene_world_coordinate(
            coordinate, "scene slice coordinate"
        )
        center = tuple(
            _required_scene_world_coordinate(value, f"scene slice center[{index}]")
        ...
        if cell.covered:
            ...
            if palette_index is None:
                ...
                key = _canonical_json_bytes(
                    block_state, description="scene slice block state"
                )
