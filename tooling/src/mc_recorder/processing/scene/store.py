...
from mc_recorder.protocol.render.sources import FLASHBACK_CAPTURE_CONTRACT
...
MIN_SCENE_WORLD_COORDINATE = SQLITE_INTEGER_MIN + MAX_SLICE_RADIUS
MAX_SCENE_WORLD_COORDINATE = SQLITE_INTEGER_MAX - MAX_SLICE_RADIUS - 1
MAX_CROP_ENTITIES = 512
MAX_CROP_BLOCK_ENTITIES = 512
MAX_CROP_OBJECT_PAYLOAD_BYTES = 256 * 1024
MAX_CROP_OBJECT_PAYLOAD_TOTAL_BYTES = 4 * 1024 * 1024
...
_EXTRACTION_PROVENANCE_FIELDS = frozenset({"scope", "metadata_policy", "result"})
...
class SceneCrop:
    ...
    def cell(self, x: int, y: int, z: int) -> tuple[bool, Any | None]:
        ...
        return self.coverage[offset], (None if palette_index == UNKNOWN_PALETTE_INDEX else self.palette[palette_index])
...
def _canonical_json_bytes(value: Any, *, description: str) -> bytes:  # noqa: ANN401
    try:
        return json.dumps(
            value,
            ensure_ascii=False,
...
def canonical_json_blob(value: Any) -> bytes:  # noqa: ANN401
...
def canonical_section_blob(palette: Sequence[Any], indices: Sequence[int]) -> bytes:
    ...
    if len(indices) != SECTION_CELL_COUNT:
        raise SceneStoreError(f"section indices must contain exactly {SECTION_CELL_COUNT} cells")
    if not palette:
        raise SceneStoreError("section palette cannot be empty")

    encoded_states = [_canonical_json_bytes(state, description=f"section palette entry {index}") for index, state in enumerate(palette)]
    ...
    for offset, source_index in enumerate(indices):
        ...
        if not 0 <= source_index < len(palette):
            raise SceneStoreError(f"section index {offset} references missing palette entry {source_index}")
...
def _decode_section_blob(data: bytes) -> tuple[tuple[Any, ...], tuple[int, ...]]:
    ...
    except (ValueError, base64.binascii.Error) as exc:  # ty:ignore[unresolved-attribute]
...
def _required_text(value: Any, description: str) -> str:  # noqa: ANN401
    if not isinstance(value, str) or not value:
        raise SceneStoreError(f"{description} must be non-empty text")
    return value


def _required_int(value: Any, description: str) -> int:  # noqa: ANN401
    if isinstance(value, bool) or not isinstance(value, int):
        raise SceneStoreError(f"{description} must be an integer")
    return value


def _required_scene_world_coordinate(value: Any, description: str) -> int:  # noqa: ANN401
    coordinate = _required_int(value, description)
    if not MIN_SCENE_WORLD_COORDINATE <= coordinate <= MAX_SCENE_WORLD_COORDINATE:
        raise SceneStoreError(f"{description} must be between {MIN_SCENE_WORLD_COORDINATE} and {MAX_SCENE_WORLD_COORDINATE}")
    return coordinate


def _required_sha256(value: Any, description: str) -> str:  # noqa: ANN401
    if not isinstance(value, str) or len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise SceneStoreError(f"{description} must be lowercase SHA-256 text")
    return value


def _finite_number(value: Any, description: str) -> float:  # noqa: ANN401
...
def _float_tuple(value: Any, length: int, description: str) -> tuple[float, ...]:  # noqa: ANN401
    if not isinstance(value, (list, tuple)) or len(value) != length:
        raise SceneStoreError(f"{description} must contain exactly {length} numbers")
    return tuple(_finite_number(item, f"{description}[{index}]") for index, item in enumerate(value))
...
def _entity_fields(
    ...
    type_id = _required_text(payload.get("type_id", payload.get("type")), "entity type_id")
    network_value = payload.get("network_id")
    network_id = None if network_value is None else _required_int(network_value, "entity network_id")
    ...
    velocity = None if velocity_value is None else _float_tuple(velocity_value, 3, "entity velocity")
    rotation_value = payload.get("rotation")
    rotation = None if rotation_value is None else _float_tuple(rotation_value, 2, "entity rotation")
...
def _block_entity_type(payload: Mapping[str, Any]) -> str:
    return _required_text(payload.get("type_id", payload.get("type")), "block entity type_id")
...
def _decompress_bounded(data: bytes, *, expected_size: int, description: str) -> bytes:
    if expected_size < 0 or expected_size > MAX_BLOB_UNCOMPRESSED_BYTES:
        raise SceneStoreValidationError(f"{description} uncompressed size is outside the allowed bounds")
    inflater = zlib.decompressobj()
    try:
        value = inflater.decompress(data, expected_size + 1)
        if len(value) > expected_size or inflater.unconsumed_tail:
            raise SceneStoreValidationError(f"{description} expands beyond its declared size")
    ...
    if len(value) != expected_size:
        raise SceneStoreValidationError(f"{description} size does not match its declared size")
    return value
...
def _blob_value(connection: sqlite3.Connection, sha256: str, *, expected_kind: str) -> bytes:
    ...
    if row["kind"] != expected_kind:
        raise SceneStoreValidationError(f"scene blob {sha256} has kind {row['kind']}, expected {expected_kind}")
    compressed = bytes(row["zlib_data"])
    if len(compressed) != row["compressed_size"]:
        raise SceneStoreValidationError(f"scene blob {sha256} compressed size does not match")
...
def _blob_uncompressed_size(connection: sqlite3.Connection, sha256: str, *, expected_kind: str) -> int:
    row = connection.execute(
    ...
    if not isinstance(size, int) or isinstance(size, bool) or not 0 <= size <= MAX_BLOB_UNCOMPRESSED_BYTES:
        raise SceneStoreValidationError(f"scene blob {sha256} uncompressed size is outside the allowed bounds")
    return size
...
def _validated_nonnegative_int(value: object, description: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool) or value < 0:
        raise SceneStoreValidationError(f"{description} must be a non-negative integer")
    return value


def _validated_source_replays(value: object, description: str = "scene source_replays") -> tuple[dict[str, Any], ...]:
    if not isinstance(value, (list, tuple)) or not value:
        raise SceneStoreValidationError(f"{description} must be a non-empty array")
    ...
    for index, raw in enumerate(value):
        ...
        if not isinstance(raw, Mapping) or set(raw) != _SOURCE_REPLAY_FIELDS:
            raise SceneStoreValidationError(f"{context} fields do not match the source replay contract")
        segment_id = _required_text(raw.get("segment_id"), f"{context} segment_id")
        ordinal = _validated_nonnegative_int(raw.get("segment_ordinal"), f"{context} segment_ordinal")
        path = _required_text(raw.get("path"), f"{context} path")
        digest = _required_sha256(raw.get("sha256"), f"{context} sha256")
        size_bytes = _validated_nonnegative_int(raw.get("size_bytes"), f"{context} size_bytes")
        if size_bytes == 0:
        ...
        if ordinal <= previous_ordinal or segment_id in segment_ids:
            raise SceneStoreValidationError(f"{description} must have unique IDs and strictly increasing ordinals")
...
def _validated_subject_poses(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != _SUBJECT_POSE_FIELDS:
        raise SceneStoreValidationError("scene extraction subject_poses fields do not match the contract")
    ...
    if not path.is_absolute() or path != path.resolve() or path.name != "subject-poses.jsonl":
        raise SceneStoreValidationError("scene extraction subject_poses path must be an absolute normalized subject-poses.jsonl")
    digest = _required_sha256(value.get("sha256"), "scene extraction subject_poses sha256")
    size_bytes = _validated_nonnegative_int(value.get("size_bytes"), "scene extraction subject_poses size_bytes")
    record_count = _validated_nonnegative_int(value.get("record_count"), "scene extraction subject_poses record_count")
    first_tick = _validated_nonnegative_int(value.get("first_tick"), "scene extraction subject_poses first_tick")
...
def _validated_extraction_provenance(
    ...
    *,
    ...
    if not isinstance(provenance, Mapping) or set(provenance) != _EXTRACTION_PROVENANCE_FIELDS:
        raise SceneStoreValidationError("scene extraction provenance fields do not match the contract")
    scope = provenance.get("scope")
    metadata_policy = provenance.get("metadata_policy")
    if scope != SCENE_SCOPE_CLIENT_VISIBLE:
        raise SceneStoreValidationError("scene extraction scope must be client_visible")
    if metadata_policy != SCENE_METADATA_POLICY_FULL:
        raise SceneStoreValidationError("scene extraction metadata_policy must be full_packet_metadata")
    result = provenance.get("result")
    if not isinstance(result, Mapping) or set(result) != _EXTRACTION_RESULT_FIELDS:
        raise SceneStoreValidationError("scene extraction result fields do not match the complete contract")
    if result.get("schema_version") != 1 or isinstance(result.get("schema_version"), bool) or result.get("result_type") != SCENE_EXTRACTION_RESULT_TYPE or result.get("status") != "complete":
        raise SceneStoreValidationError("scene extraction result contract is unsupported or incomplete")
    expected_identity = {
    ...
    if any(result.get(key) != expected for key, expected in expected_identity.items()):
        raise SceneStoreValidationError("scene extraction result identity or tick range does not match the store")
    _required_text(result.get("job_id"), "scene extraction result job_id")
    if result.get("scope") != scope or result.get("metadata_policy") != metadata_policy or result.get("flashback_capture_contract") != FLASHBACK_CAPTURE_CONTRACT:
        raise SceneStoreValidationError("scene extraction result policy or capture contract does not match persisted provenance")
    result_sources = _validated_source_replays(result.get("source_replays"), "scene extraction result source_replays")
    if result_sources != sources:
        raise SceneStoreValidationError("scene extraction result source replays do not match the store")
    ...
    if not isinstance(ignored, Mapping) or any(not isinstance(name, str) or not name or not isinstance(count, int) or isinstance(count, bool) or count < 0 for name, count in ignored.items()):
        raise SceneStoreValidationError("scene extraction result ignored_packet_counts is invalid")
    if (
        ...
        raise SceneStoreValidationError("scene extraction result does not cover every stored frame")
    stream = result.get("stream")
    if not isinstance(stream, Mapping) or set(stream) != _EXTRACTION_STREAM_FIELDS:
        raise SceneStoreValidationError("scene extraction result stream fields do not match the contract")
    if stream.get("format") != SCENE_STREAM_SCHEMA or stream.get("frames_index") != "frames.jsonl" or stream.get("changes_index") != "changes.jsonl" or stream.get("blobs_directory") != "blobs":
        raise SceneStoreValidationError("scene extraction result stream layout is unsupported")
    ...
    counts = {
        key: _validated_nonnegative_int(stream.get(key), f"scene extraction result stream {key}")
        for key in (
            "frame_count",
            "change_count",
            "blob_count",
    ...
    if counts["frame_count"] != frame_count:
        raise SceneStoreValidationError("scene extraction stream frame_count does not match the store")
    if counts["frames_size_bytes"] == 0 or counts["changes_size_bytes"] == 0:
        raise SceneStoreValidationError("scene extraction result stream indexes must be non-empty")
    return SceneExtractionProvenance(
        scope=scope,  # ty:ignore[invalid-argument-type]
        metadata_policy=metadata_policy,  # ty:ignore[invalid-argument-type]
...
def validate_scene_attachment_provenance(
    ...
    if info.extraction.metadata_policy == SCENE_METADATA_POLICY_FULL and not info.sensitive:
        raise SceneStoreValidationError("full_packet_metadata scene stores must be marked sensitive")
    return info.extraction


def _store_info(connection: sqlite3.Connection, path: Path, *, include_hash: bool) -> SceneStoreInfo:
    ...
    source_replays = _json_array_from_text(meta["source_replays_json"], "scene source_replays")
    provenance = _json_object_from_text(meta["provenance_json"], "scene provenance")
    identity = SceneIdentity(
    ...
    if extraction is not None and extraction.metadata_policy == SCENE_METADATA_POLICY_FULL and not sensitive:
        raise SceneStoreValidationError("full_packet_metadata scene stores must be marked sensitive")
    return SceneStoreInfo(
        ...
        frame_count=len(ticks),
        ticks=ticks,
        coverage_complete=connection.execute("SELECT NOT EXISTS(SELECT 1 FROM frames WHERE coverage_complete = 0)").fetchone()[0] == 1,
...
def _validate_interval_bounds(
    ...
    bad = connection.execute(
        f"SELECT start_tick, end_tick FROM {table} WHERE start_tick < ? OR end_tick > ? OR start_tick >= end_tick LIMIT 1",
        (start_tick, end_tick + 1),
    ).fetchone()
    if bad is not None:
        raise SceneStoreValidationError(f"{table} contains an interval outside the store tick bounds")
...
def _validate_no_overlap(
    ...
    *,
    ...
    if overlap is not None:
        raise SceneStoreValidationError(description or f"{table} contains overlapping intervals")
...
def _validate_entity_rtree(connection: sqlite3.Connection) -> None:
    ...
    extra = connection.execute("SELECT 1 FROM entity_versions_rtree AS search LEFT JOIN entity_versions AS source ON source.rowid = search.version_rowid WHERE source.rowid IS NULL LIMIT 1").fetchone()
    if mismatched is not None or extra is not None:
        raise SceneStoreValidationError("entity temporal/spatial index does not match entity versions")
...
def _validate_block_entity_rtree(connection: sqlite3.Connection) -> None:
    ...
    extra = connection.execute("SELECT 1 FROM block_entity_versions_rtree AS search LEFT JOIN block_entity_versions AS source ON source.rowid = search.version_rowid WHERE source.rowid IS NULL LIMIT 1").fetchone()
    if mismatched is not None or extra is not None:
        raise SceneStoreValidationError("block-entity temporal/spatial index does not match block-entity versions")


def _validate_connection(connection: sqlite3.Connection, path: Path) -> None:
    version = connection.execute("PRAGMA user_version").fetchone()[0]
    if version != SCENE_STORE_USER_VERSION:
        raise SceneStoreValidationError(f"unsupported scene store user_version {version}; expected 1")
    ...
    for table in ("section_versions", "entity_versions", "block_entity_versions"):
        _validate_interval_bounds(connection, table, meta["start_tick"], meta["end_tick"])
    ...
    for table, column, expected_kind in references:
        ...
        if wrong_kind is not None:
            raise SceneStoreValidationError(f"{table} references a blob with the wrong content kind")
    for row in connection.execute("SELECT sha256, kind FROM blobs ORDER BY sha256"):
        data = _blob_value(connection, row["sha256"], expected_kind=row["kind"])
        if row["kind"] == "section":
            _decode_section_blob(data)
        else:
            payload = _parse_canonical_json(data, description=f"{row['kind']} blob {row['sha256']}")
            if not isinstance(payload, dict):
                raise SceneStoreValidationError(f"{row['kind']} blob {row['sha256']} must contain an object")
...
def validate_scene_store(
    ...
    *,
    ...
    if info.identity != expected_identity:
        raise SceneStoreValidationError("scene store identity does not match the expected session/player/connection")
    if expected_start_tick is not None and info.start_tick != expected_start_tick:
        raise SceneStoreValidationError(f"scene store starts at tick {info.start_tick}, expected {expected_start_tick}")
    if expected_end_tick is not None and info.end_tick != expected_end_tick:
        raise SceneStoreValidationError(f"scene store ends at tick {info.end_tick}, expected {expected_end_tick}")
...
class SceneStoreBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        descriptor, temporary_name = tempfile.mkstemp(prefix="mc-recorder-scene-", suffix=".sqlite3", dir=staging)
        os.close(descriptor)
        ...
        source_json = _canonical_json_bytes(list(source_replays), description="scene source_replays").decode("utf-8")
        provenance_json = _canonical_json_bytes(dict(provenance or {}), description="scene provenance").decode("utf-8")
        ...
        self._active_entities: dict[str, tuple[int, int | None, str, str, tuple[float, ...], str]] = {}
        self._active_network_ids: dict[int, str] = {}
        self._closed_entity_instances: set[str] = set()
        self._active_block_entities: dict[tuple[str, int, int, int], tuple[int, str, str]] = {}
    ...
    def _tick(self, tick: int, *, stream: str) -> int:
        ...
        if not self.start_tick <= tick <= self.end_tick:
            raise SceneStoreError(f"scene {stream} tick {tick} is outside [{self.start_tick},{self.end_tick}]")
    ...
    def _put_blob(self, kind: str, value: bytes) -> str:
        ...
        existing = self._connection.execute("SELECT kind, uncompressed_size FROM blobs WHERE sha256 = ?", (digest,)).fetchone()
    ...
    def add_frame(
        ...
        *,
        ...
        payload_sha = self._put_blob("frame", _canonical_json_bytes(payload, description="frame payload"))
        try:
            self._connection.execute(
    ...
    def set_section_blob(
        ...
        coordinates = tuple(_required_int(value, f"section coordinate {index}") for index, value in enumerate(section_position))
    ...
    def unload_section(self, tick: int, dimension: str, section_position: Sequence[int]) -> None:
        tick = self._tick(tick, stream="change")
        dimension = _required_text(dimension, "section dimension")
        if len(section_position) != 3:
            raise SceneStoreError("section position must contain x, y, z")
    ...
    def _close_entity(self, instance_id: str, end_tick: int) -> None:
        start_tick, network_id, dimension, type_id, aabb, digest = self._active_entities.pop(instance_id)
    ...
    def set_entity_blob(self, tick: int, instance_id: str, canonical_blob: bytes) -> None:
        ...
        dimension, type_id, network_id, _uuid, _position, _velocity, _rotation, aabb = _entity_fields(instance_id, value)
        ...
        if owner is not None and owner != instance_id:
            raise SceneStoreError(f"entity network_id {network_id} is already active as {owner}")
    ...
    def _close_block_entity(self, key: tuple[str, int, int, int], end_tick: int) -> None:
    ...
    def set_block_entity_blob(
        ...
        coordinates = tuple(_required_int(value, f"block entity coordinate {index}") for index, value in enumerate(position))
    ...
    def remove_block_entity(self, tick: int, dimension: str, position: Sequence[int]) -> None:
        tick = self._tick(tick, stream="change")
        dimension = _required_text(dimension, "block entity dimension")
        if len(position) != 3:
            raise SceneStoreError("block entity position must contain x, y, z")
    ...
    def publish(
        ...
        *,
        ...
        if expected_ticks is not None:
            ticks = _validated_ticks(expected_ticks)
            actual = tuple(row[0] for row in self._connection.execute("SELECT server_tick FROM frames ORDER BY server_tick"))
            if actual != ticks:
                raise SceneStoreError("scene builder frame ticks do not match expected ticks")
        temporary_descriptor, temporary_name = tempfile.mkstemp(prefix=f".{output.name}.tmp-", suffix=".sqlite3", dir=output.parent)
...
class SceneStore:
    ...
    def _frame_row(self, tick_or_frame_id: int | str) -> sqlite3.Row:
        ...
        if isinstance(tick_or_frame_id, int):
            row = self._connection.execute("SELECT * FROM frames WHERE server_tick = ?", (tick_or_frame_id,)).fetchone()
        elif isinstance(tick_or_frame_id, str) and tick_or_frame_id:
            row = self._connection.execute("SELECT * FROM frames WHERE frame_id = ?", (tick_or_frame_id,)).fetchone()
        else:
            raise SceneStoreError("scene frame key must be an integer tick or frame id")
        if row is None:
    ...
    def _bounded_scene_object_rows(
        self,
        ...
        *,
        ...
        if len(rows) > max_count:
            raise SceneStoreError(f"scene crop {label} count exceeds the limit of {max_count}")
        total_payload_bytes = 0
        for row in rows:
            digest = row["blob_sha256"]
            payload_bytes = _blob_uncompressed_size(self._connection, digest, expected_kind=expected_kind)
            if payload_bytes > MAX_CROP_OBJECT_PAYLOAD_BYTES:
                raise SceneStoreError(f"scene crop {label} payload {digest} contains {payload_bytes} bytes; per-object limit is {MAX_CROP_OBJECT_PAYLOAD_BYTES}")
            total_payload_bytes += payload_bytes
            if total_payload_bytes > MAX_CROP_OBJECT_PAYLOAD_TOTAL_BYTES:
                raise SceneStoreError(f"scene crop {label} payloads contain more than {MAX_CROP_OBJECT_PAYLOAD_TOTAL_BYTES} bytes in total")
        return rows
    ...
    def _entities_in_box(
        ...
        rows = self._bounded_scene_object_rows(
            cursor,
            label="entity",
    ...
    def materialize_crop(
        ...
        if cell_count > min(max_cells, DEFAULT_MAX_CROP_CELLS):
            raise SceneStoreError(f"scene crop contains {cell_count} cells; limit is {min(max_cells, DEFAULT_MAX_CROP_CELLS)}")
        for index in range(3):
            maximum = origin[index] + shape[index]
            if not SQLITE_INTEGER_MIN <= origin[index] <= SQLITE_INTEGER_MAX or not SQLITE_INTEGER_MIN <= maximum <= SQLITE_INTEGER_MAX:
                raise SceneStoreError("scene crop bounds must fit signed 64-bit SQLite integers")
        ...
        for row in rows:
            section_palette, section_indices = _decode_section_blob(_blob_value(self._connection, row["blob_sha256"], expected_kind="section"))
            translated: list[int] = []
    ...
    def slice(
        ...
        if isinstance(radius, bool) or not isinstance(radius, int) or not 0 <= radius <= MAX_SLICE_RADIUS:
            raise SceneStoreError(f"scene slice radius must be an integer from 0 to {MAX_SLICE_RADIUS}")
        coordinate = _required_scene_world_coordinate(coordinate, "scene slice coordinate")
        center = tuple(_required_scene_world_coordinate(value, f"scene slice center[{index}]") for index, value in enumerate(center))  # ty:ignore[invalid-assignment]
        diameter = radius * 2 + 1
        if axis == "x":
...
def scene_slice_to_json(value: SceneSlice) -> dict[str, Any]:
    ...
    palette: list[Any] = []
    palette_lookup: dict[bytes, int] = {}
    palette_identity_lookup: dict[int, int] = {}
    cells: list[dict[str, Any]] = []
    for cell in value.cells:
        palette_index: int | None = None
        if cell.covered:
            identity = id(cell.block_state)
            palette_index = palette_identity_lookup.get(identity)
            if palette_index is None:
                block_state = _thaw_json(cell.block_state)
                key = _canonical_json_bytes(block_state, description="scene slice block state")
                palette_index = palette_lookup.get(key)
                if palette_index is None:
                    palette_index = len(palette)
                    palette_lookup[key] = palette_index
                    palette.append(block_state)
                palette_identity_lookup[identity] = palette_index
        cells.append(
            {
    ...
    with path.open("rb") as handle:
        for line_number, line in enumerate(handle, start=1):
            if len(line) > MAX_JSONL_LINE_BYTES:
                raise SceneStoreError(f"{description} line {line_number} exceeds the size limit")
            if not line.strip():
                raise SceneStoreError(f"{description} line {line_number} is empty")
            try:
                value = json.loads(line)
            except json.JSONDecodeError as exc:
                raise SceneStoreError(f"{description} line {line_number} is not valid JSON") from exc
            if not isinstance(value, dict):
                raise SceneStoreError(f"{description} line {line_number} must contain an object")
...
def _canonicalize_stream_section_blob(data: bytes) -> bytes:
    ...
    if value.get("format") == "mc-recorder-section-v1":
        ...
        return canonical
    if value.get("schema_version") != 1 or value.get("cell_order") not in {"y_z_x", SECTION_INDEX_ORDER}:
...
def _verified_scene_stream_context(
    ...
    *,
    ...
    _required_sha256(verified_stream.result_sha256, "verified scene result sha256")
    ...
    if not isinstance(stream_path, str) or Path(stream_path).resolve() != directory.resolve():
        raise SceneStoreError("verified scene result stream path does not match compaction input")
    integrity = dict(stream)
    integrity.pop("path")
    if integrity != verified_stream.integrity.as_dict():
        raise SceneStoreError("verified scene result stream envelope does not match its integrity snapshot")
    provenance = {
...
def _frame_logical_value(frame: Mapping[str, Any], line_number: int) -> dict[str, Any]:
    ...
    normalized_position = _float_tuple(position, 3, f"scene frames line {line_number} subject_position")
    return {
        "dimension": _required_text(frame.get("dimension"), f"scene frames line {line_number} dimension"),
...
def _change_resource(event_type: str, body: Mapping[str, Any]) -> tuple[int, tuple[Any, ...], dict[str, Any]]:
...
def compact_scene_stream(
    ...
    *,
    ...
    if output.exists() or output.is_symlink():
        if not force:
            raise SceneStoreError(f"scene store output exists: {output}; pass --force to replace it")
    ...
    for line_number, frame in _read_jsonl(directory / "frames.jsonl", "scene frames"):
        ...
        if tick not in tick_set:
            raise SceneStoreError(f"scene frames line {line_number} has unexpected tick {tick}")
        logical = _frame_logical_value(frame, line_number)
        if logical["metadata"].get("scope") != provenance["scope"] or logical["metadata"].get("metadata_policy") != provenance["metadata_policy"]:
            raise SceneStoreError(f"scene frames line {line_number} policy does not match the verified extraction result")
        existing = frames.get(tick)
        if existing is not None:
            if existing[1] != logical:
                raise SceneStoreError(f"overlapping scene frames disagree at global tick {tick}")
            ...
            if previous_segment != current_segment:
                ...
                if not isinstance(snapshot_hash, str) or len(snapshot_hash) != 64 or any(character not in "0123456789abcdef" for character in snapshot_hash):
                    raise SceneStoreError("overlapping scene frames require an equal logical snapshot hash")
            previous_order = (
    ...
    stage_descriptor, stage_name = tempfile.mkstemp(prefix="mc-recorder-scene-events-", suffix=".sqlite3", dir=output.parent)
    os.close(stage_descriptor)
    stage_path = Path(stage_name)
    segment_begins: dict[str, dict[str, Any]] = {}
    try:
        with closing(sqlite3.connect(stage_path)) as stage:
            stage.execute("CREATE TABLE events (server_tick INTEGER NOT NULL, priority INTEGER NOT NULL, resource_key TEXT NOT NULL, payload_json TEXT NOT NULL, PRIMARY KEY(server_tick, resource_key))")
            previous_sequence: int | None = None
            for line_number, change in _read_jsonl(directory / "changes.jsonl", "scene changes"):
                sequence_value = change.get("sequence")
                if sequence_value is not None:
                    sequence = _required_int(sequence_value, f"change line {line_number} sequence")
                    if previous_sequence is not None and sequence != previous_sequence + 1:
                        raise SceneStoreError("scene change sequence must be contiguous and increasing")
                    previous_sequence = sequence
                event_value, body = _change_body(change)
                event_type = _required_text(event_value, f"change line {line_number} type")
                if event_type == "segment_begin":
                    ...
                    if begin["segment_ordinal"] < 0 or begin["size_bytes"] < 0:
                        raise SceneStoreError(f"change line {line_number} segment provenance cannot be negative")
                    previous = segment_begins.get(segment_id)
                    if previous is not None and previous != begin:
                        raise SceneStoreError(f"conflicting segment_begin provenance for {segment_id}")
                ...
                if tick not in tick_set:
                    raise SceneStoreError(f"scene changes line {line_number} has unexpected tick {tick}")
                priority, resource, payload = _change_resource(event_type, body)
                resource_json = _canonical_json_bytes(list(resource), description="scene change resource").decode("utf-8")
                payload_json = _canonical_json_bytes(payload, description="scene change payload").decode("utf-8")
                existing = stage.execute(
                    "SELECT payload_json FROM events WHERE server_tick = ? AND resource_key = ?",
                    (tick, resource_json),
                ).fetchone()
                if existing is not None:
                    if existing[0] != payload_json:
                        raise SceneStoreError(f"overlapping scene changes disagree at tick {tick} for resource {resource_json}")
                    continue
                stage.execute(
            ...
            if not segment_begins:
                raise SceneStoreError("scene stream lacks segment_begin source replay provenance")
            known_sources = {source["segment_id"]: source for source in source_replays}
            if set(segment_begins) != set(known_sources):
                raise SceneStoreError("scene segment_begin records do not exactly match source replays")
            for segment_id, begin in segment_begins.items():
                known = known_sources[segment_id]
                if any(known[key] != begin[key] for key in ("segment_ordinal", "sha256", "size_bytes")):
                    raise SceneStoreError(f"segment_begin does not match source replay {segment_id}")
            ...
            try:
                ...
                rows = stage.execute("SELECT server_tick, payload_json FROM events ORDER BY server_tick, priority, resource_key")
                ...
                for tick, payload_json in rows:
                    ...
                    if event_type == "section_set":
                        builder.set_section_blob(
                            ...
                            _canonicalize_stream_section_blob(_stream_blob(directory, payload["blob_sha256"])),
                        )
                        ...
                        if canonical_instance is None:
                            ...
                            canonical_entity_aliases.setdefault(canonical_instance, set()).add(incoming_instance)
                        elif entity_alias_identity[incoming_instance] != identity_key:
                            raise SceneStoreError(f"scene entity alias changes identity: {incoming_instance}")
                        ...
                        canonical_instance = entity_aliases.get(incoming_instance)
                        if canonical_instance is None:
                            raise SceneStoreError(f"scene removes an unknown entity alias: {incoming_instance}")
                ...
                if existing_output is None:
                    if output.exists() or output.is_symlink():
                        raise SceneStoreError("scene store output appeared while compaction was running")
                else:
                    try:
                        output_stat = output.stat()
                    except OSError as exc:
                        raise SceneStoreError("existing scene store changed while compaction was running") from exc
                    if (
                        output.is_symlink()
                        or (
                            output_stat.st_dev,
                        ...
                        raise SceneStoreError("existing scene store changed while compaction was running")
                try:
                    verify_scene_stream(directory, verified_stream.integrity)
                except SceneStreamIntegrityError as exc:
                    raise SceneStoreError(f"scene stream changed during compaction: {exc}") from exc
