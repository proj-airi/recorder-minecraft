...
def _validate_job_payload(value: object) -> dict[str, Any]:
    ...
    if not isinstance(payload["dataset_id"], str) or not _HEX_32_RE.fullmatch(
        payload["dataset_id"]
    ):
...
def _validate_plan(value: object) -> dict[str, Any]:
    ...
    if not isinstance(plan["lease_token_sha256"], str) or not _SHA256_RE.fullmatch(
    ...
    for source in sources:
        ...
        ordinal = _bounded_integer(
            source["segment_ordinal"], "replay segment ordinal", 0, 2**31 - 1
        )
...
class RenderRpcService:
    ...
    def __init__(self, config: RecorderConfig):
        ...
        self.rpc_root = _ensure_directory(
            self.runtime / "render-rpc", "render RPC root", create=True
        )
        self.attempts_root = _ensure_directory(
            self.rpc_root / "attempts", "render RPC attempts root", create=True
        )
        self.exports_root = _ensure_directory(
            config.paths.exports, "export root", create=True
        )
        self.imports_root = _ensure_directory(
    ...
    def _claim(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        claim = self.queue.claim(
            worker_id, job_id=job_id, lease_seconds=lease_seconds
        )
        if claim is None:
    ...
    def _create_plan(
        self, claim: dict[str, Any], worker_id: str
        ...
        generation = _bounded_integer(
            attempt.get("generation"), "render attempt generation", 1, 2**31 - 1
        )
    ...
    def _pin_source(
        ...
        if source.player_uuid != payload["player_uuid"] or source.connection_id != payload[
            "connection_id"
        ]:
        ...
        try:
            ...
            raise RecorderError(
                "cannot pin replay for rendering; replay and runtime paths must share a filesystem"
            ) from exc
    ...
    def _fence(
        self, plan: Mapping[str, Any], worker_id: str, attempt_id: str, lease_token: str
    ) -> dict[str, Any]:
    ...
    def _request(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        cutoff = (
            _bounded_integer(cutoff_value, "newer cutoff", 0, 2**63 - 1)
        ...
        for source in plan["sources"]:
            ...
            if skip_path.exists() or skip_path.is_symlink():
                ...
                marker = _strict_object(
                    _read_json(
                        skip_path, MAX_RPC_BODY_BYTES, "render segment skip marker"
                    ),
                    required={
                        "schema_version",
                        "skip_type",
    ...
    def _import_source(
        self, plan: Mapping[str, Any], root: Path, source: Mapping[str, Any]
    ) -> ImportedRenderResult:
        request_path = root / source["request_file"]
        replay = root / source["replay_file"]
        upload = root / source["upload_directory"]
        ...
        _assert_safe_descendant(
            self.exports_root, job_import_root, "durable render job import directory"
        )
        digest, size = _sha256_file(replay, "pinned replay source")
        if digest != source["sha256"] or size != source["size_bytes"]:
            raise RecorderError("pinned replay source failed its plan integrity envelope")
    ...
    def _finalization_value(
        plan: Mapping[str, Any], imports: list[ImportedRenderResult]
    ) -> dict[str, Any]:
        return {
            "schema_version": 1,
    ...
    def _finalize_response(
        self, plan: Mapping[str, Any], root: Path, job: Mapping[str, Any]
    ) -> dict[str, Any]:
        value = _read_json(
            root / "finalize.json", MAX_FINALIZE_BYTES, "remote render finalization"
        )
        _strict_object(
            value,
            required={
        ...
        if (
            ...
            or value["finalize_type"] != FINALIZE_TYPE
            or value["owner"] != PLAN_OWNER
            or value["job_id"] != plan["job_id"]
            or value["attempt_id"] != plan["attempt_id"]
            or value["worker_id"] != plan["worker_id"]
        ):
        ...
        expected_ids = [
            source["segment_id"] for source in self._requested_sources(plan, root)
        ]
        actual_ids = [
            item.get("segment_id") if isinstance(item, dict) else None
        ...
        for item in value["imports"]:
            ...
            source = next(
                source for source in plan["sources"] if source["segment_id"] == item["segment_id"]
            )
            directory = self.exports_root / source["import_directory"]
            _assert_safe_descendant(
                self.exports_root, directory, "durable render finalized import"
            )
    ...
    def _response_from_values(
        plan: Mapping[str, Any], job: Mapping[str, Any], imports: list[dict[str, Any]]
    ) -> dict[str, Any]:
        return {
            "job": dict(job),
            "identity": {
            ...
            "attachment_directories": [
                item["directory"] for item in imports if item["status"] == "complete"
            ],
        }


def dispatch_render_rpc(
    config: RecorderConfig, action: str, body: Mapping[str, Any]
) -> dict[str, Any]:
