...
GRADLE_EXECUTABLE_ENV = "MC_RECORDER_GRADLE"
...
def _resolve_gradle_executable(environment: dict[str, str]) -> str:
    override = environment.get(GRADLE_EXECUTABLE_ENV)
    if override is not None:
        candidate = Path(override)
        if not candidate.is_absolute():
            raise RecorderError(f"{GRADLE_EXECUTABLE_ENV} must be an absolute executable path")
        if not candidate.is_file() or not os.access(candidate, os.X_OK):
            raise RecorderError(f"{GRADLE_EXECUTABLE_ENV} is not an executable file: {candidate}")
    ...
    if discovered is None:
        raise RecorderError("Gradle executable not found; run `proto install --config-mode local` and expose its shim on PATH, or set MC_RECORDER_GRADLE to an absolute executable path")
