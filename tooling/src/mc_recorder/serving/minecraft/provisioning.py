...
from mc_recorder.processing.capture.storage import StorageReport, enforce_quota
...
def _build_recorder_mod(config: RecorderConfig) -> None:
    ...
    build_environment = current_process_environment()
...
def write_mod_configs(config: RecorderConfig) -> None:
    ...
    _write_json(config_root / "server-replay" / "config.json", server_replay)  # ty:ignore[invalid-argument-type]
    ...
    _write_json(config_root / "mc-recorder.json", capture)  # ty:ignore[invalid-argument-type]
...
def prepare_runtime(config: RecorderConfig, *, build_mod: bool) -> Path | None:
    for directory in (
        ...
        config.paths.runtime / "control" / "render-ready",
    ):
...
def _compose_environment(config: RecorderConfig) -> dict[str, str]:
    return compose_runtime_environment(config)
    ...
    *,
    ...
    with operation_lock(config.paths.runtime, "server_start"):
        if not config.server.eula:
            raise RecorderError("Minecraft EULA has not been accepted; review https://aka.ms/MinecraftEULA and set server.eula=true or rerun init with --accept-eula")
