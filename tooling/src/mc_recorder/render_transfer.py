...
from .episodes import sha256_file, validate_episode
...
def _integer(value: Any, context: str, minimum: int, maximum: int = 2**63 - 1) -> int:
    if (
...
def _validate_request(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(episode.get("manifest_sha256"), str) or _SHA256_RE.fullmatch(
...
def write_portable_render_request(
...
def materialize_portable_render_job(
    ...
    *,
    ...
    if resolved_output.exists() or resolved_output.is_symlink():
        if not force:
            raise RecorderError(
                f"render job output exists: {resolved_output}; pass force=True to replace it"
            )
        if (
            resolved_output.is_symlink()
            or not resolved_output.is_dir()
            or not _owned_render_directory(resolved_output)
        ):
...
def _raw_result_range(
    result: Mapping[str, Any], request: PortableRenderRequest
...
def _load_bundle(
    ...
    *,
    ...
    if not isinstance(manifest.get("payload_index_sha256"), str) or _SHA256_RE.fullmatch(
        manifest["payload_index_sha256"]
    ) is None:
    ...
    for line_number, line in enumerate(index_bytes.splitlines(), 1):
        ...
        _required_keys(
            row, {"path", "sha256", "size_bytes"}, set(), f"payload index line {line_number}"
        )
...
def _assert_payload_inventory(
    staging: Path, payload: list[_PayloadFile], request: PortableRenderRequest, status_value: str
) -> None:
...
def _validate_imported(
    ...
    for relative, metadata in files.items():
        ...
        payload.append(
            _PayloadFile(relative, int(metadata["size_bytes"]), str(metadata["sha256"]))
        )
...
def import_render_bundle(
    ...
    *,
    ...
    staging = Path(
        tempfile.mkdtemp(
            prefix=f".{resolved_destination.name}.tmp-", dir=resolved_destination.parent
        )
    )
    try:
        for item in payload:
        ...
        if first_tick != bundle_manifest["global_start_tick"] or last_tick != bundle_manifest[
            "global_end_tick"
        ]:
        ...
        manifest_path = _write_import_manifest(
            staging, portable, bundle_sha, status_value
        )
        try:
