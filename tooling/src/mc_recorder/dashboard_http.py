...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler contract
        ...
        try:
            ...
            if path == "/api/v1/server/start":
                ...
                if "replace_legacy_rgb" in body and not isinstance(
                    body["replace_legacy_rgb"], bool
                ):
    ...
    def _scene_slice_payload(cls, scene_slice: object) -> dict[str, Any]:
        ...
        for cell in cells:
            ...
            block_state = cell.get("block_state")
            colored_cells.append(
                {
                    **cell,
                    "color": cls._block_color(block_state)
                    if cell["covered"] and block_state is not None
                    else None,
                }
            )
