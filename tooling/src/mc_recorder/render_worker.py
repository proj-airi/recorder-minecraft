...
_INITIAL_ERROR_RETRY_SECONDS = 2.0
_MAX_ERROR_RETRY_SECONDS = 30.0
...
class _IncompatibleServerError(RecorderError):
    """The remote server cannot safely support a persistent worker."""
...
class _WorkerPresence:
    """Keep a persistent process visible while a server-side finalize blocks."""

    def __init__(self, remote: RemoteRecorder, worker_id: str):
        self.remote = remote
        self.worker_id = _canonical_uuid(worker_id, "render worker ID")
        self._stop = threading.Event()
        self._thread = threading.Thread(
            target=self._heartbeat_loop,
            name="mc-recorder-worker-presence",
            daemon=True,
        )
    ...
    def _heartbeat_loop(self) -> None:
        while not self._stop.wait(_WORKER_HEARTBEAT_SECONDS):
            try:
                ...
                # Claim/lease calls remain authoritative and surface errors to
                # the foreground loop. Presence is display-only.
                pass
    ...
    *,
    ...
    if persistent and (
        ...
        raise _IncompatibleServerError(
            ...
            "update the server tooling or use --once"
        )
...
def _run_registered_worker_once(
    ...
    *,
    ...
    try:
        ...
        raise _ClaimedJobError(
            "render worker stopped because the remote claim outcome is unknown; "
            "inspect the queue before restarting"
        ) from exc
        ...
        if reason == "claim_failed":
            try:
                error = _bounded_string(
                    claimed.get("error"), "claimed job failure", 2048
                )
                failed_job = claimed.get("failed_job")
                if not isinstance(failed_job, dict):
                    raise RecorderError("remote render claim lacks its failed job")
                failed_job_id = _canonical_uuid(
                    failed_job.get("id"), "failed render job ID"
                )
            except RecorderError as exc:
                raise _ClaimedJobError(
                    "server reported a claimed job failure with invalid details; "
                    "inspect the queue before restarting"
                ) from exc
            raise _ClaimedJobError(
                f"server failed claimed job {failed_job_id} before transfer: {error}"
            )
        if reason is not None and reason != "replay_pending":
            raise RecorderError(f"remote render claim returned an unsupported reason: {reason!r}")
        deferred_cooldown = claimed.get("deferred_job_cooldown_seconds")
        if deferred_cooldown is not None and (
            reason != "replay_pending"
            or not isinstance(deferred_cooldown, int)
            or isinstance(deferred_cooldown, bool)
            or not 1 <= deferred_cooldown <= 300
        ):
            raise RecorderError("remote render claim returned an invalid deferred-job cooldown")
    ...
    try:
        ...
        if isinstance(raw_claim, dict):
            ...
            if claimed_job_id is None and isinstance(raw_job, dict):
                try:
                    claimed_job_id = _canonical_uuid(
                        raw_job.get("id"), "claimed render job ID"
                    )
                except RecorderError:
                    pass
                ...
                if attempt_id is None:
                    try:
                        attempt_id = _canonical_uuid(
                            raw_attempt.get("id"), "render attempt ID"
                        )
                    except RecorderError:
                        pass
        ...
        raise _ClaimedJobError(
            f"render worker stopped after claimed job {identity} returned an invalid response: {detail}"
        ) from exc
    ...
    try:
        ...
        failure = _ClaimedJobError(
            f"render worker stopped after job {claimed_job_id} failed: {detail}"
        )
            ...
            try:
                ...
                if primary_error is not None:
                    primary_error.add_note(
                        f"render workspace cleanup also failed: {detail}"
                    )
                else:
                    raise _ClaimedJobError(
                        f"render worker stopped after job {claimed_job_id} cleanup failed: {detail}"
                    ) from cleanup_exc
...
def run_ephemeral_worker(
    ...
    *,
    ...
    _register_worker(remote, worker_id, persistent=False)
...
def run_render_worker(
    ...
    *,
    ...
    """Poll and process render jobs sequentially under one worker identity.

    The default mode runs until interrupted. ``once`` retains the old
    one-claim behavior, while selecting an exact ``job_id`` always stops after
    that claim so it can never fall through to unrelated queue work.
    """

    if (
        not isinstance(poll_interval, (int, float))
        or isinstance(poll_interval, bool)
        or not 1.0 <= float(poll_interval) <= MAX_POLL_INTERVAL_SECONDS
    ):
        raise RecorderError(
            f"render worker poll interval must be between 1 and {int(MAX_POLL_INTERVAL_SECONDS)} seconds"
        )
    if not isinstance(once, bool):
        raise RecorderError("render worker once flag must be a boolean")
    selected_job = _canonical_uuid(job_id, "render job ID") if job_id is not None else None
    stop_after_one = once or selected_job is not None
    persistent = not stop_after_one
    worker_id = ephemeral_worker_id()
    cache = default_worker_cache() if cache_root is None else cache_root
    registered = False
    error_retry = _INITIAL_ERROR_RETRY_SECONDS
    presence = _WorkerPresence(remote, worker_id) if persistent else None
    ...
    try:
        while True:
            if not registered:
                try:
                ...
                except _IncompatibleServerError:
                    raise
                except RecorderError as exc:
                    if stop_after_one:
                        raise
                    ...
                    if on_error is not None:
                        on_error(str(exc), delay)
                    waiter(delay)
                    error_retry = min(error_retry * 2.0, _MAX_ERROR_RETRY_SECONDS)
                    continue
                registered = True
                if presence is not None and not presence_started:
                    presence.start()
            ...
            except _ClaimedJobError:
                # A missing Java runtime, broken renderer build, full disk, or
                # another host-wide fault could fail every queued job. Stop after
                # the first claimed failure and leave later jobs untouched.
                raise
            except RecorderError as exc:
                if stop_after_one:
                    raise
            ...
            if stop_after_one:
                return None
            if (
                poll.reason == "replay_pending"
                and poll.deferred_job_cooldown_seconds is not None
            ):
                # The server just put this exact job on an eligibility cooldown.
                # Claim again immediately so later ready jobs are not delayed; if
                # none exist, the next generic empty response takes the normal
                # poll sleep and prevents a busy loop. Older servers omit the
                # retry hint, so they take the normal sleep instead of hot-looping.
                continue
            waiter(float(poll_interval))
    finally:
        if presence is not None and presence_started:
            presence.stop()
