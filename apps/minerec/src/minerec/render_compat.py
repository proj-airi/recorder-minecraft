...
def validate_unsupported_packet_summary(value: object, label: str = "renderer unsupported_packets") -> dict[str, Any]:
    ...
    if not isinstance(total, int) or isinstance(total, bool) or not 0 <= total <= MAX_UNSUPPORTED_PACKET_COUNT:
    ...
    for index, entry in enumerate(types):
        ...
        if not isinstance(packet_type, str) or len(packet_type) > 256 or _PACKET_TYPE_RE.fullmatch(packet_type) is None or packet_type <= previous:
            raise RecorderError(f"{entry_label} packet_type is invalid or unsorted")
        if not isinstance(count, int) or isinstance(count, bool) or not 1 <= count <= MAX_UNSUPPORTED_PACKET_COUNT:
