...
from mc_recorder.config import RecorderConfig
from mc_recorder.errors import RecorderError
from mc_recorder.processing.capture.episodes import resolve_episode
from mc_recorder.processing.capture.exporter import CANONICAL_RENDER_RESULT_TYPE, export_episode
from mc_recorder.processing.dataset.viewer import DatasetMetadata, DatasetViewer, DatasetViewerError, opaque_dataset_id
from mc_recorder.processing.render.hud import validate_hud_result_envelope
from mc_recorder.protocol.render.contract import FULL_CLIENT_PRESENTATION_CONTRACT
from mc_recorder.protocol.render.transfer import PORTABLE_REQUEST_TYPE, RENDER_IMPORT_TYPE, ImportedRenderResult
...
def _job_identity(config: RecorderConfig, job: Mapping[str, Any]) -> _JobIdentity:
    ...
    selection_start_tick = _required_tick(payload.get("selection_start_tick", start_tick), "selection start tick")
    selection_end_tick = _required_tick(payload.get("selection_end_tick", end_tick), "selection end tick")
    ...
    output = config.paths.exports / f"{session_id}-{player_uuid}-{connection_id}.dataset"
...
def _verify_dataset(config: RecorderConfig, identity: _JobIdentity) -> _VerifiedDataset:
    ...
    if (
        ...
        raise RecorderError("refusing to overwrite dataset with a conflicting session or connection selection")
...
def _validate_import(
    ...
    if result.get("schema_version") != 2 or result.get("result_type") != CANONICAL_RENDER_RESULT_TYPE or result.get("artifact_root") != ".":
    ...
    if presentation == FULL_CLIENT_PRESENTATION_CONTRACT:
        ...
        structured_hud = validate_hud_result_envelope(result.get("structured_hud"), "render import structured_hud")
        if structured_hud["start_server_tick"] != identity.start_tick or structured_hud["end_server_tick"] != identity.end_tick:
            raise RecorderError("render import structured_hud range does not match its queue job")
        if (
            structured_hud["dataset_id"] != identity.dataset_id
            or structured_hud["dataset_manifest_sha256"] != dataset.manifest_sha256
            ...
            raise RecorderError("render import structured_hud does not match the currently verified dataset")
    elif result.get("structured_hud") is not None:
        raise RecorderError("render import structured_hud lacks the current presentation contract")
    ...
    if portable.get("request_type") != PORTABLE_REQUEST_TYPE or not isinstance(request_sha, str) or _SHA256_RE.fullmatch(request_sha) is None:
    ...
    if segment_id != directory.name or (supplied_segment is not None and supplied_segment != segment_id):
    ...
    if first_tick > last_tick or first_tick < identity.start_tick or last_tick > identity.end_tick:
...
def _result(
    ...
    return RenderAttachmentResult(
        ...
        state_count=metadata.state_count,  # ty:ignore[invalid-argument-type]
        action_count=metadata.action_count,  # ty:ignore[invalid-argument-type]
