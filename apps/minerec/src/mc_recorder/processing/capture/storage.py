...
from mc_recorder.config import load_runtime_env
from mc_recorder.errors import RecorderError
from mc_recorder.processing.capture.episodes import directory_size, inspect_epoch
...
def _completed_replay(path: Path, root: Path, *, stable_before_ns: int) -> bool:
    ...
    try:
        ...
        with zipfile.ZipFile(candidate, "r") as archive:
            ...
            is_flashback = "metadata.json" in names and any(name.endswith(".flashback") for name in names)
...
def completed_replay_paths(
    ...
    *,
    ...
    candidates = [path for path in root.rglob("*") if _completed_replay(path, root, stable_before_ns=stable_before_ns)]
    ...
    try:
        for candidate in sorted(epochs.iterdir()):
            ...
            if (
                ...
                or current.get("sealed") is not True
            ):
                fcntl.flock(handle.fileno(), fcntl.LOCK_UN)
                handle.close()
...
def _evict_replay(replays_root: Path, replay: Path, *, stable_seconds: int) -> EvictedEpoch | None:
    root = replays_root.resolve()
...
def _replays_root(captures_root: Path, explicit: Path | None) -> Path:
    ...
    configured = load_runtime_env().replay_root
...
def enforce_quota(
    ...
    *,
    ...
    if before >= quota_bytes and evict_oldest:
        candidates = [_Candidate("capture_epoch", path, _epoch_sort_timestamp(path)) for path in sealed_epoch_paths(capture_root)]
        candidates.extend(_Candidate("replay_archive", path, _modified_ns(path)) for path in completed_replay_paths(replay_root, stable_seconds=replay_stable_seconds))
        ...
        for candidate in candidates:
            ...
            if candidate.source_kind == "capture_epoch":
                ...
                replay_eviction = _evict_replay(replay_root, candidate.path, stable_seconds=replay_stable_seconds)
    ...
    if after >= quota_bytes:
        status = "full"
        if evict_oldest:
            message = "capture and replay storage is full; no more verified sealed epochs or stable completed replay archives can be safely evicted"
        else:
