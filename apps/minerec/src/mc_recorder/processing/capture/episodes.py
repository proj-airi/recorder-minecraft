...
def inspect_epoch(path: Path) -> EpochInfo | None:
    ...
    manifest = _read_json(manifest_path) if manifest_path.is_file() and not manifest_path.is_symlink() else None
    ...
    return EpochInfo(
        ...
        events_bytes=_manifest_int(manifest, "events_bytes"),
        events_sha256=(str(manifest["events_sha256"]).lower() if isinstance(manifest.get("events_sha256"), str) and re.fullmatch(r"[0-9a-fA-F]{64}", str(manifest["events_sha256"])) else None),
    )
    ...
    *,
    ...
    if manifest is None:
        ...
        if raw_session_id != episode.name:
            issues.append(ValidationIssue("warning", str(manifest_path), "manifest session_id differs from directory name"))
    ...
    for epoch in epoch_infos:
        ...
        if expected_count is not None:
            ...
            if actual_count != expected_count:
                issues.append(ValidationIssue("error", relative, f"manifest event count {expected_count} does not match {actual_count}"))
        ...
        if expected_bytes is not None:
            ...
            if actual_bytes != expected_bytes:
                issues.append(ValidationIssue("error", relative, f"manifest byte count {expected_bytes} does not match {actual_bytes}"))
