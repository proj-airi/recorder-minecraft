...
def _is_sha256(value: object) -> bool:
    return isinstance(value, str) and len(value) == 64 and all(character in _SHA256_CHARACTERS for character in value)
...
def scene_stream_integrity_from_mapping(value: Mapping[str, Any]) -> SceneStreamIntegrity:
    ...
    return SceneStreamIntegrity(
        ...
        frames_size_bytes=_required_nonnegative_int(value.get("frames_size_bytes"), "frames_size_bytes"),
        frames_sha256=frames_sha256,  # ty:ignore[invalid-argument-type]
        frame_count=_required_nonnegative_int(value.get("frame_count"), "frame_count"),
        changes_index="changes.jsonl",
        changes_size_bytes=_required_nonnegative_int(value.get("changes_size_bytes"), "changes_size_bytes"),
        changes_sha256=changes_sha256,  # ty:ignore[invalid-argument-type]
...
def _referenced_blobs(changes: bytes, *, first_line: int = 1) -> set[str]:
    ...
    for line_number, line in enumerate(changes.splitlines(), start=first_line):
        try:
            ...
            raise SceneStreamIntegrityError(f"scene changes line {line_number} is not valid JSON") from exc
        if not isinstance(value, dict):
            raise SceneStreamIntegrityError(f"scene changes line {line_number} must contain an object")
        ...
        if not _is_sha256(digest):
            raise SceneStreamIntegrityError(f"scene changes line {line_number} has an invalid blob SHA-256")
        referenced.add(digest)
    return referenced


def _verify_blobs(directory: Path, referenced: set[str], expected: SceneStreamIntegrity) -> None:
    try:
        if directory.is_symlink() or not directory.is_dir():
            raise SceneStreamIntegrityError("scene blobs must be a regular non-symlink directory")
    ...
    for entry in entries:
        ...
        try:
            uncompressed = inflater.decompress(compressed, MAX_BLOB_UNCOMPRESSED_BYTES + 1)
        ...
        if len(uncompressed) > MAX_BLOB_UNCOMPRESSED_BYTES or not inflater.eof or inflater.unused_data:
    ...
    if actual != referenced:
        raise SceneStreamIntegrityError("scene blob files do not exactly match the blobs referenced by changes")
