...
def _scene_store(
    ...
    *,
    ...
    subject_positions: dict[int, tuple[float, float, float]] | None = None,
) -> Path:
...
class EpisodeExportTest(unittest.TestCase):
    ...
    def test_rejects_scene_pose_mismatch_before_dataset_publication(self) -> None:
        cases = {
            "dimension": (
                {"dimension": "minecraft:the_nether"},
                {},
            ),
            "subject-position-outside-selection": (
                {
                    "subject_positions": {
                        10: (1.0000000000000002, 64.0, 2.0),
                        11: (1.1, 64.0, 2.0),
                    },
                },
                {"first_tick": 11},
        ...
        for name, (scene_options, export_options) in cases.items():
            with self.subTest(name=name), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                output = root / "dataset"
                with self.assertRaisesRegex(
                    RecorderError,
                    "dimension .* does not match|subject_position .* does not match",
                ):
                    export_episode(
                        _episode(root),
                        output,
